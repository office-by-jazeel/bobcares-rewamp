    ...
    <>
      ...
      >
        ...
        <div className="relative z-10">
          ...
          {/* Pin circle */}
          <div className="relative w-6 h-6 rounded-full bg-black border border-[#FFFFFF63] group-hover:bg-white group-hover:border-black transition-colors duration-400">
            {/* Plus icon - visible by default */}
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none"
              className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 scale-[75%] transition-opacity duration-300 opacity-100 group-hover:opacity-0">
            ...
            {/* Minus icon - visible on hover */}
            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="2" viewBox="0 0 12 2" fill="none"
              className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 scale-[75%] transition-opacity duration-300 opacity-0 group-hover:opacity-100">
      ...
      {clients && clients.length > 0 && (
        <div
          ref={tooltipRef}
          className="fixed w-fit flex items-start flex-col gap-4 px-6 pt-4.5 pb-3 bg-[#222222] backdrop-blur-[70px] border border-[#FFFFFF2B] rounded-[16px] rounded-bl-none shadow-lg transition-opacity duration-300 pointer-events-none z-9999"
          style={{
    ...
    <section className="bg-black overflow-hidden relative">
      <div className="container mx-auto flex flex-col lg:flex-row items-start h-full w-full py-14 lg:py-[140px] gap-12 lg:gap-16">
        ...
        <div className="relative flex-1 w-full md:hidden flex flex-col gap-2.5">
          {mapPinsData.mapPins.flatMap((pin) =>
            pin.clients.map((client, clientIndex) => (
              <div key={`${pin.id}-${clientIndex}`} className="flex items-center justify-between border border-[#FFFFFF2B] bg-[#131313] rounded-[14px] px-4 py-5">
                {client.logo ? (
                  <CloudinaryImage
                    src={client.logo}
                    alt={client.label}
