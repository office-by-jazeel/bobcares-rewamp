...
interface MapPinProps {
  top: string | number; // Percentage or pixel value (e.g., "20%", 100)
  left: string | number; // Percentage or pixel value (e.g., "30%", 200)
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        {/* Pin circle */}
        <div className="relative w-9 h-9 rounded-full bg-black border border-[#FFFFFF63] group-hover:bg-white group-hover:border-black transition-colors duration-300">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none"
            className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 block group-hover:hidden">
        ...
        {label && (
          <div className="absolute bottom-full left-0 transform w-[300px] mb-2 px-6 pt-5 pb-4 bg-[#222222] backdrop-blur-[70px] border border-[#FFFFFF2B] rounded-[16px] rounded-bl-none shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
            {logo && (
            ...
            <span className="text-[#FFFFFF96] text-base leading-normal">
              {label}
            </span>
          </div>
        )}
  ...
  return (
    <section className="bg-black overflow-hidden relative">
      <div className="container mx-auto flex flex-col lg:flex-row items-start h-full w-full py-14 lg:py-[140px] gap-12 lg:gap-16">
        ...
        {/* World Map Graphic */}
        <div className="relative flex-1 mt-32 w-full">
          <CloudinaryImage
          ...
          {/* Map Pins - Add pins by passing position props */}
          <MapPin top="25%" left="20%" label="North America" />
          <MapPin top="30%" left="50%" label="Europe" />
          <MapPin top="45%" left="55%" label="Middle East" />
          <MapPin top="60%" left="75%" label="Asia Pacific" />
          <MapPin top="70%" left="30%" label="South America" />
          <MapPin top="50%" left="15%" label="Africa" />
        </div>
      </div>
    </section>
  );
