      ...
      {clients && clients.length > 0 && (
        ...
        >
          {clients.map((client, index) => (
            <div key={index} className="flex flex-col gap-1 border-b border-b-[#ffffff18] py-4.5 first:pt-0 last:pb-0 last:border-b-0 w-full">
              {client.logo ? (
                <CloudinaryImage
                  src={client.logo}
                  alt={'label' in client && client.label ? client.label : "Client logo"}
                  className="object-contain h-6 w-fit filter invert brightness-0"
                />
              ) : (
